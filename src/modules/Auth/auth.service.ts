...
const userLogin = async(payload: Record<string, unknown>) => {
    ...
    const token = jwt.sign({id: user.rows[0].id, name: user.rows[0].name, email: user.rows[0].email, phone:user.rows[0].phone, role: user.rows[0].role}, config.Jwt_secret as string, {expiresIn: '1h'});
